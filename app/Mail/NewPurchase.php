...
class NewPurchase extends Mailable
{
    ...
    public function build()
    {
        ...
        if($this->is_admin){
            $subject = sprintf('Order Sent Successfully for outlet %s by user %s',$this->outlet->name, Auth::user()->name);
            $data['admin'] = $this->admin;
        }else if($this->is_supplier){
            $subject = sprintf('Receive an Order by outlet %s of Store %s',$this->outlet->name,$this->store->name);
        }else{
            $subject = sprintf('Order Sent Successfully for outlet %s',$this->outlet->name);
        }

        return $this->subject($subject)->view('email.newPurchase', $data);
    }
}