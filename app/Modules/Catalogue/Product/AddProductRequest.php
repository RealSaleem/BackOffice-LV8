...
//             dd($request);
            ...
            if($request->is_combo != -1){
                $validateQTY = $this->validateQtyAndOutOfStock($request);
                if($validateQTY == false){
                    return new Response(false,null,null,['Please add product quantity']);
                }
            }
        }
            ...
            if(is_array($request->product_images) && count($request->product_images) > 0)
            {
            ...
            return new Response(true, $product,null,null,\Lang::get('toaster.item_added'));
    ...
    private function mapProduct($request)
    {
        ...
        $product->name                  = isset($request->title_en) ? $request->title_en : $request->en_name ;
        ...
        if(isset($request->is_item) && $request->is_item){
            $product->is_item    = $request->is_item;
        ...
        if ($request->sku_type == 1) {
            ...
            $product->prefix  = $request->sku_custom;
    ...
    private function mapVariants($variants, $product_id,$request){
            ...
            foreach ($variants as $v) {
                ...
                $variant->allow_out_of_stock        = (bool)$request->allow_out_of_stock;
    ...
    private function mapComposite($composite_products, $product_id){
        ...
        foreach ($composite_products as $cp) {
            ...
            $variant->allow_out_of_stock = $request->allow_out_of_stock;
    ...
    private function validateQtyAndOutOfStock($request){
        $valid = true;
        foreach($request->branches as $outlet => $value){
            if(($value['quantity'] <= 0 || $value['quantity'] == null) && $request->allow_out_of_stock == null) {
                $valid = false;
                break;
            }
        }
