    ...
    protected static function processImages($store_id)
    {
        ...
        if (isset($theme->sizes) && $theme->sizes != null) {
            ...
            if (is_array($product_image_sizes)) {
                ...
                //dd(DB::logging());
                //dd( DB::connection('mysql')->disableQueryLog() );
                //DB::connection()->disableQueryLog();
                // Get all product images and product variants of a given store id
                $products = Product::select('id')->with(['product_images' => function($q){ $q->select('id','product_id','url','url_encoded','name'); }, 'product_variants' => function($q){
                    $q->select('product_id','sku');
                }])
                            ...
                            if (sizeof($product['product_images']) > 0) {
                                ...
                                foreach ($product['product_images'] as $image) {
                                    ...
                                    if(file_exists($url) && strtolower($file_parts[1]) != 'webp'){
                                        ...
                                        $variants = array_values(array_filter($product['product_variants'],function($variant) use($image){
                                            if($variant['product_id'] == $image['product_id']){
                                                return $variant;
                                            }
                                        }));
