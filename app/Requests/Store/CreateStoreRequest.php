...
class CreateStoreRequestValidator
{
    public function GetRules()
    {
        return [
            ...
            'store_logo'              => 'string|max:1000',
        ];
    }
}

class CreateStoreRequestHandler
{
            ...
            */
            ...
            if (is_null($user->store_id) || empty($user->store_id))
            {
                ...
                if($is_new_store){
                    ...
                    $role->description  = 'Store Admin'; // optional
                    ...
                    foreach ($permissions as $permission) {
                        ...
                        // $role->attachPermission($permission,$role->id);
                    }
            ...
            $this->setupRequired($user, $store, $request);

            $this->optionalSetup($user ,$store, $request);
    ...
    private function optionalSetup($user, $store , $request)
    {
    ...
    private function setupRequired($user, $store, $request)
    {
        $store_id = $store->id;
        ...
        if(is_null($outlet)){
            ...
            $outlet->street_1               = $request->address;
            $outlet->is_active              = 1;
            $outlet->pickup                 = 1;
            $outlet->allow_online_order     = 1;
            ...
            if(is_null($register)){
                ...
                $newRegister->name                       = $request->name.' Register';
                $newRegister->save();
            }
        }
