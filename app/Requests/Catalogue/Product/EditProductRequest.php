...
class EditProductRequestValidator
{
    public function GetRules($request)
    {
        ...
        return [
            ...
            //         return $query->where('store_id', Auth::user()->store_id)->whereNull('deleted_at');
...
class EditProductRequestHandler
{
    ...
    public function Serve($request)
    {
        ...
        return new Response(true,null,null, \Lang::get('toaster.product_updated'));
    ...
    private function mapProduct($product, $request){
        ...
        $product->is_composite          =$request->is_combo == 1;
    ...
    private function  addSingleVeirant($product,$request)
    {
        ...
        foreach ($veriants as $index => $verient) {
            // $verientariant                 = ProductVariant::find($v['id']);
            // dd($verient);
            $verient->name                      = $product->name;
            $verient->allow_out_of_stock        = (bool)$request->allow_out_of_stock == 1;
            ...
            $verient->attribute_value_1     = $product->name;
             ...
             if ($request->sku_type == 1) {
                ...
                if($err == false){
                    ...
                    $verient->sku                   =  $this->getSku($request,$index);
                    // return false;
                }
            }elseif ($request->sku_type == 0){
    ...
    private function AddProductstock($product,$request)
    {
        ...
        $parts = array_chunk($request->branches,5);

        $branches = [];

        foreach($parts as $part){
            array_push($branches, $this->array_flatten($part));
        }

        // dd($branches);

        foreach($branches as $outlet => $value)
        {

            if($product->is_composite == 1)
            {
                $productStock = ProductStock::where(['outlet_id' => $value['id'] , 'variant_id' => $variant_id, 'type' => OrderType::CompositionIn ])->first();
            }else{
                $productStock = ProductStock::where(['outlet_id' => $value['id'] , 'variant_id' => $variant_id , 'type' => OrderType::In ])->first();
            }
            if($productStock != null){
                ...
                $productStock->outlet_id         = $value['id'];
                ...
                $newProductStock->invoice_date      = date('Y-m-d H:i:s');
                // $newProductStock->invoice_number    = $request->invoice_number;
                // $newProductStock->purchase_order    = $request->purchase_order;
                $newProductStock->cost_price        = $value['supply_price'];
                $newProductStock->sale_price        = $request->retail_price;
                $newProductStock->margin            = $this->calculateMargin($newProductStock->cost_price,$request->retail_price);
                // $newProductStock->notes             = $request->notes;
                $newProductStock->quantity          = $value['quantity'] < 0 ? 0 : $value['quantity'];
                $newProductStock->re_order_quantity = $value['re_order_quantity'];
                $newProductStock->re_order_point    = $value['re_order_point'];
                $newProductStock->product_id        = $product->id;

                $newProductStock->variant_id        = $variant_id;

                $newProductStock->updated             = Auth::user()->id;
                $newProductStock->updated_at              = date('Y-m-d H:i:s');
                $newProductStock->outlet_id         = $value['id'];

                if ($product->is_composite == 1)
                {
                    $newProductStock->type              = OrderType::CompositionIn;
                }else{
                    $newProductStock->type              = OrderType::In;
                }

                $newProductStock->is_default        = 1;

                $newProductStock->is_remove = false;
