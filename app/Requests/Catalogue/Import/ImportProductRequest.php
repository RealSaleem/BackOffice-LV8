...
namespace App\Requests\Catalogue\Import;
...
class ImportProductRequestHandler {
  ...
  public function __construct()
    ...
    public function validateSheet($sheets)
    {
        ...
        if($sheets[0]->getTitle() !== null){
            if(strtolower($sheets[0]->getTitle()) !== "standard"){
                $errors['Standard Sheet'] = 'First sheet name must be Standard.';
            }
            if(strtolower($sheets[1]->getTitle()) !== "composite"){
                $errors['Composite Sheet'] = 'Second sheet name must be Composite.';
            }
            if(count($sheets) > 2){
                if(strtolower($sheets[2]->getTitle()) !== "images"){
                    $errors['Image Sheet'] = 'Third sheet name must be Images.';
                }
            }
        }else{
            $errors['Invalid sheet Uploaded'] = 'Please upload the downloaded sheet from step one';
        ...
        if(!is_null($sheet) && sizeof($sheet) > 0)
        {
            ...
            foreach($sheet as $row){
                ...
                $validator = Validator::make($row, [
                    ...
                    'sku' => ['required','min:3','max:250', Rule::notIn($skus)],
                    'variantsku' => ['required','min:3','max:250',Rule::in($sku_collection)],
                    'category' => 'required|string|nullable',
    ...
    private function validateImagesSheet($sheet,$sku_collection){
        ...
        if(!is_null($sheet) && sizeof($sheet) > 0)
        {
            ...
            foreach($sheet as $row){
                ...
                $validator = Validator::make($row, [
                    'sku' => ['required','min:3','max:250', Rule::in($sku_collection)],
    ...
    {
        $attri = ProductAttribute::with('values')->where('product_id' , $product->id)->get();
        foreach ($attri as $attrb) {
            ProductAttributeValue::where('product_attribute_id' , $attrb->id)->delete();
        }
        ProductAttribute::with('values')->where('product_id' , $product->id)->delete();
