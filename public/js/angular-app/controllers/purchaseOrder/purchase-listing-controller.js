posApp.controller('purchaseListingCtrl',[
	'$scope','$sce','storeDataService',
	'supplierDataService','brandDataService',
	'productDataService','urlService','purchaseDataService','outletDataService',
	function($scope, $sce,storeDataService,supplierDataService, brandDataService, productDataService,urlService,purchaseDataService,outletDataService){
	$scope.products = [];
	$scope.orders = [];
	$scope.outlets = [];
	$scope.suppliers = [];
 ...
 //    }
	...
	*/
	...
	$scope.reset = function(filters){
		$scope.filters = { status: 1 };
		setTimeout(() => { $("#S_reset").select2().trigger("change");  },100);
	...
	function getOrders(filters){
		$scope.fetchingProducts = true;
		purchaseDataService.list(filters).then(function(response){
		    $scope.orders = response.data.map((orders) => {
		        // product.show_variant = false;
		        return orders;
		    });
		    $scope.Next = response.next_page_url;
 ...
 //    	$scope.button.disabled = true;
	...
	//         	$('#LoaderDiv').hide();
	//             toastr.success('Stock has been updated successfully', 'Stock Updated');
