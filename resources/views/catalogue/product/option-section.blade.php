<ul class="nav nav-tabs nav-justified">
    ...
    @foreach($model->languages as $language)
        <li class="nav-item greybg1">
            <a class="tab-section nav-link show {{ ($index == 0) ? 'active' : '' }}" data-lang="{{ $language['name'] }}" data-toggle="tab" href="#option-{{ $language['name'] }}">
            {{ $language['name'] }}
            </a>
        </li>
    @php
...
<div class="tab-content mt-4">
    ...
    $counter = 0;
    @endphp
    <div id="option-{{ $language['name'] }}" class="tab-pane fade show {{ ($index == 0) ? 'active' : '' }}">
        <div class="card-body">
            <div class="row">
                ...
                @if($index == 0)
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            @if($index == 0)
                            required
                            @endif value="{{ old($name,$model->addon['code']) }}" >
                        </div>
                    </div>
                @endif
            </div>
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="form-group">
                            ...
                            @if($index == 0)
                            <span style="color: red">*</span>
                            <select class="select2 form-control" onchange="fillOptionFormPrice(this)"
                                id="{{ $language['short_name'] }}-{{ $counter }}-option_name"
                    ...
                    @if($index == 0)
                        <div class="col-md-5">
                            <div class="form-group">
                                ...
                                <input
                                id="{{ $language['short_name'] }}-{{ $counter }}-option_price"
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <button type="button" class="btn-sm btn-primary " onclick="renderOptionAddOnItems()">{{ __('backoffice.add_another_item') }}</button>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
