                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="common_title">
                            <h1>
                                {{ __('backoffice.permissions') }}


                                    <a href="{{ route('permission.create')}}"class="m-b-xs w-auto btn-primary btn-sm pull-right">
                                        {{ __('backoffice.add_role_perm') }}
                                ...
                                <div class=" table-responsive">
                                    <table class="table table-striped" id="role-table">
                                      ...
                                      <th>{{ __('backoffice.name') }} </th>
                                            <th>{{ __('backoffice.slug') }} </th>
                                            <th>{{ __('backoffice.module') }} </th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            roleTable = $('#role-table').DataTable({
                ...
                "deferLoading" : 0,
                "ajax":{
                    "url": "{{ route('api.fetch.permission') }}",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                    }
                ...
                "columns": [
                    ...
                    { data: 'actions' , sortable : false , render : function(column,row,data){
                      ...
                      @if( $permission::chekStatus('roles_edit','admin'))
                                <a href="{{url('usermanagement/permission/edit/${data.id}') }}" class="badge btn-primary">
                          {{ __('backoffice.edit') }}
                                </a>
@endif
